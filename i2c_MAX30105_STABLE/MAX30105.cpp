...
//                                                      // ... see MSP_EXP432P401R.h around Line 91 to do this enum
    ...
    System_printf("Successfully initialized the MAX30105! Start trying to get data!\n");
    System_flush();
...
void MAX30105::softReset(I2C_Handle i2c) {
  ...
  Task_sleep(1);     // delay(1); //Let's not over burden the I2C bus
}
...
bool MAX30105::safeCheck(uint8_t maxTimeToCheck)
{
  ...
  // uint32_t markTime = System_tick();

    /* === NEED TO COME UP WITH A WAY TO DEAL WITH MISSING millis() IN THIS FUNCTION */
//  while(1)
//  {
...
//    Task_sleep(1);
    ...
    // For now I'm just going to allow this to return TRUE
    return(true);
//  }
}
...
uint16_t MAX30105::check(I2C_Handle i2c)
{
  ...
  if (readPointer != writePointer)
  {
    ...
    // NEEED TO REWRITE THIS USING MSP432 I2C WRITE/READ ROUTINES
...
//      _i2cPort->requestFrom(MAX30105_ADDRESS, toGet);
      ...
      while (toGet > 0)
      {
        ...
        uint32_t temp[sizeof(uint32_t)]; //Array of 4 bytes that we will convert into long   // was type 'byte' if uint32_t causes problems - DKH
        uint32_t tempLong;
...
//        memcpy(&tempLong, temp, sizeof(tempLong));         // THIS MAY BE UNNECESSARY SINCE I MAKE EVERYTHING uint32_t ABOVE //
...
/* ============ IMPORT BIGTIME CLOCK FUNCTIONALITY TO THIS PROJECT SO THERE'S A USABLE TIME ========= */
...
//                                                           //
//  ======================= taskFxn =======================  //
//  Task for this function is created statically. See the    //
//  project's .cfg file for more details.                    //
...
Void taskFxn(UArg arg0, UArg arg1) {
    ...
    if (useMAX30105Class) {
        ...
        System_printf("Beginning mock sampling cycle.\n");
        System_flush();
        ...
        if (i2c == NULL) {
            ...
            if (partID == 0x15) {
                System_printf("PartID = %d. Successfully initialized the MAX30105! Start trying to get data!\n", partID);
                System_flush();
            } else {
                System_printf("PartID = %d. ", partID);
                System_abort("That's not the MAX30105 you're talking to... Bailing!\n");
                System_flush();
            }
        ...
        particleSensor.setLEDMode(i2c, MAX30105_MODE_MULTILED);
        ...
        uint16_t sampleRate = 100; //Options: 50, 100, 200, 400, 800, 1000, 1600, 3200
        ...
        for (iterate = 1; iterate < 20; iterate++) {
            System_printf("| Green: %d | Red: %d | NIR: %d | ", particleSensor.getGreen(i2c), particleSensor.getRed(i2c), particleSensor.getIR(i2c) );
            System_printf("TempC = %f°C or %f°F |\n", particleSensor.readTemperature(i2c), particleSensor.readTemperatureF(i2c));
            System_flush();
        }

        System_printf("Taking a five second nap, then shutting down LEDs and closing I2C bus.\n");

        Task_sleep(10000);  // wait 5 seconds

        particleSensor.setPulseAmplitudeGreen(i2c, 0x00); //Turn off Red LE
...
int main(void)
{
    ...
    Board_initI2C();
     ...
     // Construct a MAX30105 Task thread
...
//
//void readRegisterS8(I2C_Handle handle, uint8_t regAddr, int8_t *data, size_t length)
//{
//    uint8_t             txBuffer[2];
//    I2C_Transaction     i2cTransaction;
//
...
//
//void readRegisterU8(I2C_Handle handle, uint8_t regAddr, uint8_t *data, size_t   length)
//{
//    uint8_t             txBuffer[2];
//    I2C_Transaction     i2cTransaction;
//
...
//
//void writeRegister8(I2C_Handle handle, uint8_t regAddr, uint8_t value)
//{
//    uint8_t             txBuffer[2];
//    I2C_Transaction     i2cTransaction;
//
...
/////////  writeRegisterU8 for UNSIGNED 8-bit status registers (e.g.: MAX30105 tempFrac value)
//
//void writeRegister8R(I2C_Handle handle, uint8_t regAddr, uint8_t value, uint8_t readReg)
//{
